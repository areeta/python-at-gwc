...
class Scroller(object):
	...
	def add_buildings(self):
		...
		while current_width <= self.width:
			build = self.width + current_width
			self.add_building(build)
			current_width += self.building_list[-1].width
			
	def add_building(self, x_location):
		building_height = random.randint(((self.base - self.height) // 4), (self.base - self.height - 1))
		building_width = 100
		y_location = self.base - building_height
		self.building_list.append(Building(x_location, y_location, building_width, building_height, self.color))
	...
	def move_buildings(self):
		...
		for j in self.building_list:
			j.move(self.speed)
		w = self.building_list[-1].width
		lastbuilding = w + self.building_list[-1].x_point
		self.add_building(lastbuilding)
